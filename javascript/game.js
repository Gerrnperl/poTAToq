import { Layer, StaticComponent, Player, poTATo } from "./components.js";
...
export class Game {
    constructor(container, start) {
        ...
        this.background = new StaticComponent('background', loadTexture("image/background.png"), new P(0, 'width'), new P(0, 'height'), new P(100, 'width'), new P(100, 'height'));
        this.layers.background.addComponent(this.background);
        this.player = new Player('player', loadTexture("image/player.svg"), loadTexture("image/player-bullet.svg"), 40, 100, new P(50, 'width'), new P(90, 'height'), new P(5, 'height'), new P(6, 'height'));
        changeBlood(40);
    ...
    update(timeStamp) {
        ...
        if (this.player.blood > 0) {
            ...
            this.timeStamp = timeStamp !== null && timeStamp !== void 0 ? timeStamp : 0;
            requestAnimationFrame(this.update.bind(this));
        }
    }
    generateEnemy(enemyIndex = 1) {
        if (enemyIndex < 0)
            enemyIndex = 0;
        if (enemyIndex > poTATo.length - 1)
            enemyIndex = poTATo.length - 1;
        let enemyClass = poTATo[enemyIndex];
        let enemy = new enemyClass(undefined, new P(Math.random() * 100, 'width'), new P(-8, 'height'), new P(6, 'height'), new P(6, 'height'), this.player);
        enemy.vy = 0.1;
        enemy.ay = 0.01;
